...
class Thumb_info:
    ...
    def __init__(self, descr="Emptyy", order_date="Emptyy", price="Emptyy", product_id="Emptyy", title="Emptyy",
                 url="Emptyy"):
...
def getting_Time(pr_time):
    ...
    if (time_now_minute - product_time_minute) < last_desired_minutes and (
            time_now_hour - product_time_hour) == last_desired_hour:
        return True
...
def retrieve_data(my_data, cityList, vip_or_not, mail):
    for x in my_data:

        # Kutaisi:3579, Rustavi:3566, Batumi: 3533, Tbilisi: 320:
        if x['loc_id'] in cityList and getting_Time(x['order_date']):

            if vip_or_not == 'no' and int(x['vip']) == 0:
                ...
                # get ALL listings, which added last [last_desired_minutes] minutes
                descr = x['descr']
                order_date = x['order_date']
                price = x['price']
                product_id = x['product_id']
            ...
            if DTBS.restricted_accounts(user_id):  # check if product has been added by restricted account
                ...
                if DTBS.ID_exists(unique_ID):  # check if a product exist in my database, not to add again.
                    ...
                    Send_Mail.send_mail(str(product_id), mail)
